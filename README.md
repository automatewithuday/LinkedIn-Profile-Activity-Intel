...
| Row has `502` with `collection unverified: no posts returned and the run log was unavailable` | Apify returned the data but not the run log, and this profile had nothing in that source; without the log we can't tell "no posts" from "skipped" | Re-run those profiles, or check the run in Apify Console |
